...
export function DashboardSummary({ assets }: { assets: InvestorCreditAsset[] }) {
    ...
    return (
        <SimpleGrid columns={{ base: 1, md: 3 }} gap={6}>
            <Stat.Root bg="gray.800" p={5} borderRadius="md" borderWidth="1px" borderColor="gray.700">
                <Stat.Label display="flex" alignItems="center" gap={2}><Icon as={PiWallet} /> Total Investido</Stat.Label>
                <Stat.ValueText fontSize="3xl">{formatCurrency(totalInvested)}</Stat.ValueText>
            </Stat.Root>
            <Stat.Root bg="gray.800" p={5} borderRadius="md" borderWidth="1px" borderColor="gray.700">
                <Stat.Label display="flex" alignItems="center" gap={2}><Icon as={PiChartLineUp} /> Saldo Atual Total</Stat.Label>
                <Stat.ValueText fontSize="3xl">{formatCurrency(totalCurrentValue)}</Stat.ValueText>
            </Stat.Root>
            <Stat.Root bg="gray.800" p={5} borderRadius="md" borderWidth="1px" borderColor="gray.700">
                <Stat.Label display="flex" alignItems="center" gap={2}><Icon as={PiArrowRight} /> Rendimento Total</Stat.Label>
